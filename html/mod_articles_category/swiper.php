...
<div class="section-inner <?php echo $params->get('moduleclass_sfx'); ?>">
	...
	<div class="container swiper-slide<?php echo $moduleclass_sfx; ?>"><div class="row">
		<?php 
			$numberColumn = $params->get('article_column',4);
			$count = 1;
		?>
		...
		<div class="item col-xs-12 col-sm-<?php echo 12/$numberColumn; ?> <?php if($count == $numberColumn): echo 'latest'; endif; ?>">
            ...
            <?php
            $db = JFactory::getDbo();
            $db->setQuery('SELECT id FROM #__warranty_specs WHERE product='.$item->id);
            if($db->loadResult()){?>
                <div><a style="font-weight: 100" href="<?php echo JRoute::_('index.php?option=com_warranty&view=compare&id='.$item->id, false);?>">So sánh</a></div>
            <?php }?>
